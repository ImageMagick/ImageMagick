...
static MagickBooleanType WriteCIPImage(const ImageInfo *image_info,Image *image)
{
  ...
  */
  ...
  for (y=0; y < (ssize_t) image->rows; y++)
  {
    ...
    {
      byte=(unsigned char)
        ((((size_t) (4*ClampToQuantum(GetPixelIntensity(image,p+3))/QuantumRange) & 0x03) << 6) |
         (((size_t) (4*ClampToQuantum(GetPixelIntensity(image,p+2))/QuantumRange) & 0x03) << 4) |
         (((size_t) (4*ClampToQuantum(GetPixelIntensity(image,p+1))/QuantumRange) & 0x03) << 2) |
         (((size_t) (4*ClampToQuantum(GetPixelIntensity(image,p+0))/QuantumRange) & 0x03) << 0));
    ...
    if ((image->columns % 4) != 0)
      {
        ...
        byte=(unsigned char)
          ((((size_t) (4*ClampToQuantum(GetPixelIntensity(image,p+MagickMin(i,3)))/QuantumRange) & 0x03) << 6) |
           (((size_t) (4*ClampToQuantum(GetPixelIntensity(image,p+MagickMin(i,2)))/QuantumRange) & 0x03) << 4) |
           (((size_t) (4*ClampToQuantum(GetPixelIntensity(image,p+MagickMin(i,1)))/QuantumRange) & 0x03) << 2) |
           (((size_t) (4*ClampToQuantum(GetPixelIntensity(image,p+MagickMin(i,0)))/QuantumRange) & 0x03) << 0));
        (void) FormatLocaleString(buffer,MaxTextExtent,"%02x",~byte);
        (void) WriteBlobString(image,buffer);
      }
