...
static struct
  ...
  } Methods[] =
  {
    ...
    { "BilateralSmoothing", { {"geometry", StringReference},
      {"radius", RealReference}, {"sigma", RealReference},
...
Mogrify(ref,...)
  ...
  ALIAS:
    ...
    BilateralSmoothing    = 305
    BilateralSmoothingImage = 306
    MogrifyRegion      = 666
  PPCODE:
  {
    AffineMatrix
      ...
      switch (ix)
      {
        ...
        case 153:  /* BilateralSmoothing */
        {
          ...
          if (attribute_flag[5] != 0)
            channel=(ChannelType) argument_list[5].integer_reference;
          channel_mask=SetImageChannelMask(image,channel);
          image=BilateralSmoothingImage(image,geometry_info.rho,
            geometry_info.sigma,geometry_info.xi,geometry_info.psi,exception);
          if (image != (Image *) NULL)
            (void) SetImageChannelMask(image,channel_mask);
          break;
