...
CFLAGS="$CFLAGS -fPIC -I$WORK/include" cmake . -DCMAKE_INSTALL_PREFIX=$WORK -DENABLE_STATIC=true -DENABLE_SHARED=false -DWITH_SYSTEM_ZLIB=ON -DWITH_TOOLS=OFF -DWITH_TESTS=OFF
make -j$(nproc)
make install
popd
