  ...
  void *context)
{
  ...
  #pragma omp parallel for schedule(static,4) shared(progress,status) \
    magick_chunk_threads(source_image,destination_image,height,1)
#endif
  ...
  GetWandViewMethod get,void *context)
{
  ...
  height=(size_t) (source->extent.height-source->extent.y);
  #pragma omp parallel for schedule(static,4) shared(progress,status) \
    magick_chunk_threads(source_image,source_image,height,1)
#endif
  for (y=source->extent.y; y < (ssize_t) source->extent.height; y++)
  {
    const int
  ...
  SetWandViewMethod set,void *context)
{
  ...
  #pragma omp parallel for schedule(static,4) shared(progress,status) \
    magick_chunk_threads(destination_image,destination_image,height,1)
#endif
